...
| 属性名      | 值/是否             | 备注                          |
| -------- | ---------------- | --------------------------- |
| 名称       | maximum_lifespan | 最大生命周期，这是一个令牌               |
| Path     | Yes              | 全站有效                        |
...
```
我们需要生成一个`maximum_lifespan_token`，此token生成逻辑如下，并包含以下信息：

| 信息   | 键/值      | 备注                             |
| ---- | -------- | ------------------------------ |
| 用户名  | username | 用户Github的用户名与CoCo-Community的相同 |
| 登录时间 | time     | 用户登录的时间戳                       |
| ID   | id       | Github的账户ID                    |

此Cookie传递到后端后，我们校验用户Github的token获取用户名是否与`maximum_lifespan_token`匹配，若不匹配，终止流程。  
以下为`校验maximum_lifespan Cookie的token的值是否有效`完整流程，是第一个工作流的扩展。
```mermaid
flowchart TD
A[流程：校验maximum_lifespan Cookie的token的值是否有效]-->B{检查JWT签名是否有效}
B --> |是| D{校验maximum_lifespan Cookie token中的用户名是否与用户的github token匹配}
B -->|否| E[流程终止]
D -->|是| F[放行]
D -->|否| E[流程终止]
...
```
#### 终止流程的触发及相关操作
当终止流程事件触发后，将会自动进行以下操作
```mermaid
flowchart TD
A[终止事件触发]-->B{客户端是否有Cookie}
B-->|有|C[清除用户所有Cookie]
B-->|无|D[事件结束]

```

如上图所表示，当终止事件触发后，会清空用户所有的Cookie。
# 因安全问题废弃的的方案
