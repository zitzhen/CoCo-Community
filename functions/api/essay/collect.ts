...
export const onRequest: PagesFunction<{ DB: D1Database }> = async (context) => {
  ...
  const jwtTokenMatch = cookieHeader.match(/(?:^|;\s*)maximum_lifespan=([^;,\s]+)/);
  ...
  const githubTokenMatch = cookieHeader.match(/(?:^|;\s*)token=([^;,\s]+)/);
  ...
  try {
    ...
    // 验证JWT中的用户名与GitHub用户名是否匹配
    if (decodedToken.username !== username) {
      return new Response(
        JSON.stringify({ status: "error", message: "Username mismatch between JWT and GitHub" }),
        { status: 401, headers: corsHeaders }
      );
    }
    ...
    // 检查用户是否已经收藏了此文章
    const existingCollect = await env.DB.prepare(
      "SELECT * FROM essay_collect WHERE username = ? AND essayid = ?"
    ...
    if (existingCollect) {
      return new Response(
        JSON.stringify({ 
          status: "error", 
          message: "Essay already collected by this user" 
        }),
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    const result = await env.DB.prepare(
      "INSERT INTO essay_collect (username, essayid, time) VALUES (?1, ?2, ?3)"
    ...
    if (result.success) {
      return new Response(
        JSON.stringify({ 
          status: "success", 
          message: "Essay collected successfully" 
        }),
        { status: 200, headers: corsHeaders }
      );
    } else {
      return new Response(
        JSON.stringify({ status: "error", message: "Failed to save collection to database" }),
